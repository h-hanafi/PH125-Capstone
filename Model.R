...
b_g <- train %>% left_join(b_i, by= "movieId") %>% left_join(b_u, by = "userId") %>% 
  group_by(genres) %>%
  summarize(b_g = sum(rating - mu - b_i - b_u)/(n()+l), 
            sd = sqrt(sum((rating- mu - b_i - b_u) - b_g)^2/(n()-1))) 

#Visualising the new Genre effect
b_g %>% ggplot(aes(genres, b_g, ymax = b_g + sd, ymin = b_g - sd)) + geom_point() + geom_errorbar() + theme(axis.text.x = element_text(angle = 90, hjust = 1))

#Visualising the new Genre effect after filtering movies without listed genres
b_g %>% filter(genres != "(no genres listed)") %>%
  ggplot(aes(genres, b_g, ymax = b_g + sd, ymin = b_g - sd)) + geom_point() + geom_errorbar() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
b_y <- train %>% left_join(b_i, by = "movieId") %>% left_join(b_u, by = "userId") %>% left_join(b_g, by = "genres") %>%
  group_by(year) %>%
  summarize(b_y = sum(rating - mu - b_i - b_u - b_g)/(n()+l), sd = sqrt(sum((rating- mu - b_i - b_u - b_g) - b_y)^2/(n()-1)))

b_y %>% ggplot(aes(year, b_y, ymax = b_y + sd, ymin = b_y - sd)) + 
  geom_point() + geom_errorbar() + theme(axis.text.x = element_text(angle = 90, hjust = 1))

b_y %>% mutate(year = as.numeric(year)) %>% ggplot(aes(x = year,y = b_y)) + 
